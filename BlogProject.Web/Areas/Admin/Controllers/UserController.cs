...
namespace BlogProject.Web.Areas.Admin.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Create(UserCreateVM userCreateVM)
        {
            ...
            var result = await _userService.CreateUser(userCreateVM);   //bu metottan bir result döner. metottaki işlemlerin sonucuna göre başarılı dönen result if içine girer. Başarısız dönmüşse else içine girer.
            if (result.Succeeded)
            {
            ...
            {
                //BLL-FluentValidation içine "AddToIdentityModelState" metodunu yazdık.
                result.AddToIdentityModelState(this.ModelState);    
                validation.AddToModelState(this.ModelState);        //BLL-FluentValidation
                return View(new UserCreateVM { Roles = roles });
            }
        }
